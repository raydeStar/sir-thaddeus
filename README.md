...
| A model could describe a correct file change without ever producing the requested state, or claim success after a failed shell edit. | Permission-gated `file_write` and `file_replace` perform one exact UTF-8 effect inside an allowed root, then independently reread it and return byte count, SHA-256, and bounded post-state content. Ambiguous replacements, executable extensions, oversized files, reparse traversal, and unverifiable writes fail closed. | Ordinary document/config edits gain a mechanically verified completion boundary without a stronger model or benchmark-specific logic. |
...
| Scoped files | File reads and exact verified text/config edits stay inside user-configured roots and limits; writes require permission and fail closed on unsafe extensions or paths. |
