...
const tests = [
    ...
    },
    {
        ...
        run: async () => {
            ...
            const snapshotResult = await core.invokeTool({
                name: "health.get_daily_snapshot",
                args: { date }
            });
            const baseline = await core.invokeTool({
                name: "health.get_baselines",
                args: { date }
            });
            const brief = await core.invokeTool({
            ...
            const similar = await core.invokeTool({
                name: "health.get_similar_past_days",
                args: { date }
            });
            const jobBrief = await core.runJob("health.morning_strategy_job");
    ...
    },
    {
        ...
        run: async () => {
            ...
            assertEqual(auth.lifecycle, "not_configured");
    ...
    },
    {
        ...
        run: async () => {
            ...
            const result = await runtime.completeProviderAuth({ code: "auth-code" });
    ...
    },
    {
        ...
        run: async () => {
            ...
            const brief = await client.callTool("health.get_morning_strategy_brief", { date: "2026-06-02" });
    ...
    },
    {
        ...
        run: async () => {
            ...
            try {
                ...
                assert(content.some((item) => item.type === "text" && item.text?.includes("\"date\": \"2026-06-02\"")), "stdio MCP brief should include the requested date");
                assert(!statusContent.some((item) => item.text?.includes("GOOGLE_HEALTH_CLIENT_SECRET=")), "stdio MCP provider status should not expose secret values");
            }
            finally {
                await client.close();
                await rm(dir, { recursive: true, force: true });
...
function assertFlags(signals, flags) {
