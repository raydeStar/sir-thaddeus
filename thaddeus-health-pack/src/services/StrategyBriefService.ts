...
import { SimilarPastDaysService } from "./SimilarPastDaysService.js";
...
export class StrategyBriefService {
  ...
  constructor(private readonly similarPastDays: SimilarPastDaysService) {}

  async create(snapshot: DailyHealthSnapshot, baseline: HealthBaseline): Promise<StrategyBrief> {
    const signals = this.detector.detect(snapshot, baseline);
    const dayType = this.detector.classifyDay(signals, snapshot, baseline);
    const bodySignals = signals.filter((signal) =>
      ["sleep_low", "sleep_very_low", "hrv_low", "resting_hr_high", "hard_workout", "soreness_high", "low_activity"].includes(signal.flag)
    );
    const subjectiveSignals = signals.filter((signal) =>
      ["stress_high", "mood_low", "protein_low", "drift_risk"].includes(signal.flag)
    );
    ...
    return {
      ...
      disclaimer:
        "This is a practical planning brief based on recent patterns. For severe, persistent, or unusual symptoms, use qualified health support."
    };
  }

  toPatternEpisode(brief: StrategyBrief): PatternEpisode {
...
function contributors(signals: HealthSignalComparison[]): StrategyContributor[] {
  return signals
...
function readinessLevel(dayType: DayType): StrategyBrief["readinessLevel"] {
  if (dayType === "gold") return "strong";
  if (dayType === "red") return "recovery";
  if (dayType === "yellow" || dayType === "blue") return "caution";
  return "normal";
}

function baselineComparisons(snapshot: DailyHealthSnapshot, baseline: HealthBaseline): StrategyBrief["baselineComparisons"] {
  return [
...
function comparison(
  ...
  unit: string
): StrategyBrief["baselineComparisons"][number] {
...
function expectation(dayType: DayType, signals: HealthSignalComparison[]): string {
...
function recommendations(dayType: DayType, signals: HealthSignalComparison[]): string[] {
  const flags = new Set(signals.map((signal) => signal.flag));
  const recs: string[] = [];

  if (flags.has("protein_low")) recs.push("Get protein and water before noon.");
  if (flags.has("sleep_very_low") || flags.has("sleep_low")) recs.push("Do one focused work block before side quests.");
  if (flags.has("hard_workout") || flags.has("soreness_high")) recs.push("Train moderately today; no PR attempt.");
  if (flags.has("prior_day_load_high")) recs.push("Reduce training ambition and bias toward recovery movement.");
  if (flags.has("low_activity") || flags.has("drift_risk")) recs.push("Take a 20-minute walk to create momentum.");
  if (flags.has("stress_high") || flags.has("mood_low")) recs.push("Pick the smallest useful plan and reduce optional commitments.");
  if (dayType === "gold") recs.push("Use the strong window for one important deep-work block.");
  recs.push("Start wind-down earlier tonight.");

  return [...new Set(recs)].slice(0, 5);
}

function nonNegotiable(dayType: DayType, signals: HealthSignalComparison[]): string {
  const flags = new Set(signals.map((signal) => signal.flag));
  if (flags.has("protein_low")) return "Protein and water before noon.";
  if (flags.has("drift_risk")) return "One focused work block before distractions.";
  if (dayType === "red") return "Keep training and workload conservative today.";
  if (dayType === "blue") return "Use a simple schedule anchor before checking the day by mood.";
  return "Protect one meaningful work block.";
}

function caveats(snapshot: DailyHealthSnapshot): string[] {
...
function similarSummary(days: SimilarPastDay[]): string | undefined {
  if (days.length === 0) {
