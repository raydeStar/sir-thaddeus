...
export function createHealthPackStdioServer(): McpServer {
  const server = new McpServer({
    name: "thaddeus-health-pack",
    version: healthPackManifest.version
  });

  registerJsonTool(server, "health.get_daily_snapshot", "Get the cached daily health snapshot for a date.", async ({ date }) =>
    runtime.getDailySnapshot(date ?? todayIso()));

  registerJsonTool(server, "health.refresh_daily_snapshot", "Refresh and store a daily health snapshot from the provider.", async ({ date }) =>
    runtime.refreshDailySnapshot(date ?? todayIso()));

  registerJsonTool(server, "health.get_baselines", "Calculate rolling personal health baselines for a date.", async ({ date }) =>
    runtime.getBaselines(date ?? todayIso()));

  registerJsonTool(server, "health.get_morning_strategy_brief", "Generate the practical morning strategy brief for a date.", async ({ date }) =>
    runtime.getMorningStrategyBrief(date ?? todayIso()));

  registerJsonTool(server, "health.get_similar_past_days", "Find prior days with overlapping health flags.", async ({ date }) =>
    runtime.getSimilarPastDays(date ?? todayIso()));

  registerJsonTool(server, "health.provider_status", "Return sanitized health provider configuration/auth status.", async () =>
    runtime.getProviderStatus());

  registerJsonTool(server, "health.provider_config_schema", "Return the sanitized provider setup schema.", async () =>
    runtime.getProviderConfigSchema());

  registerJsonTool(server, "health.secret_store_status", "Return the local secret protection backend status.", async () =>
    runtime.getSecretStoreStatus());
  ...
  registerJsonTool(server, "health.clear_provider_config", "Reset provider configuration to mock and clear stored tokens.", async () =>
    runtime.clearProviderConfig());
  ...
  registerJsonTool(server, "health.disconnect_provider", "Disconnect the configured provider and clear stored tokens.", async () =>
    runtime.disconnectProvider());
...
function registerJsonTool(
  ...
  name: string,
  description: string,
  handler: (args: { date?: string }) => Promise<unknown>
): void {
