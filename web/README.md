...
Sir Thaddeus web workspace. React 18 + Vite 5 + TanStack Router (file-based) +
Zustand + Tailwind 3.
...
Phase-1 stubs are live for every route in spec §19:
...
| `/automations`, `/automations/:id`, `/automations/new` | `routes/automations*.tsx` |
| `/compact` | `routes/compact.tsx` (quick-interaction panel) |

`src/routeTree.gen.ts` is generated by `@tanstack/router-plugin/vite` and is
gitignored; running `npm run build` (or `npm run dev`) regenerates it.
