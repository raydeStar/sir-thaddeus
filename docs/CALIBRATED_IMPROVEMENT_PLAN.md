...
| ---: | --- | --- | --- |
| Validated locally; PR pending | Attested verified file-effect completions | Product quality | With Gemma 31B fixed, fresh development retained exact state at `8/8`, preserved `4/4` eligible deterministic completions, left `2/2` richer response contracts model-authored, removed one protocol artifact, and reduced calls `30 -> 18`, tokens `28,616 -> 18,433`, and measured test work `150.438 s -> 87.793 s`. The exact candidate repeated at `8/8`, `4/4`, and `18` calls. Disjoint validation retained both arms at canonical and mechanical `8/8`, again activated `4/4` with zero inappropriate activation, and reduced calls `30 -> 18`, tokens `28,698 -> 19,493`, and test work `137.065 s -> 91.613 s`. Full local tests passed `3,082` with two skips. This is redundant-finalization removal after externally verified state, not a capability or MMLU gain. |
