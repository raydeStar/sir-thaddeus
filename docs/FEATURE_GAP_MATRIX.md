...
| Knowledge-store MCP hooks | `Partial` | Root listing, file reads, file listing, file creation, append, and journal-entry tools exist | Tool surface exists, but there is no dedicated workspace UI for knowledge-store management | [packages/mcp-tools-core/SirThaddeus.McpTools.Core/Tools/KnowledgeStoreMcpTools.cs](../packages/mcp-tools-core/SirThaddeus.McpTools.Core/Tools/KnowledgeStoreMcpTools.cs), [packages/mcp-shared/SirThaddeus.McpShared/ToolManifest.cs](../packages/mcp-shared/SirThaddeus.McpShared/ToolManifest.cs) |
| User memo workspace | `Complete` | Memo CRUD with tags and pinning | Distinct from the lower-level agent memory store | [src/Thaddeus.Runtime/Api/MemoryApi.cs](../src/Thaddeus.Runtime/Api/MemoryApi.cs), [web/src/routes/memory.tsx](../web/src/routes/memory.tsx) |
