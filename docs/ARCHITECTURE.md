...
| Files and knowledge | File list/read with preview/apply pattern, exact verified UTF-8 write/replace, document extraction, allowlisted roots, and wiki tools | File access is permission-gated and root-scoped; writes fail closed outside conservative text/config extensions |
...
| Files and knowledge store | `file_read`, `file_read_preview`, `file_read_apply`, `file_write`, `file_replace`, `document_read`, `file_list`, `file_list_preview`, `file_list_apply`, `knowledge_store_list_roots`, `knowledge_store_read_file`, `knowledge_store_list_files`, `knowledge_store_create_file`, `knowledge_store_append_to_file`, `knowledge_store_journal_log_entry` | `file_write` atomically writes exact UTF-8 content; `file_replace` changes one unique exact span. Both independently reread the result and return a bounded post-state receipt. They require the files permission, stay inside allowed roots, cap output at 1 MiB, reject reparse traversal, and default to `.json`, `.yaml`, `.yml`, `.toml`, `.ini`, `.env`, `.md`, and `.txt` when no extension policy is supplied. |
