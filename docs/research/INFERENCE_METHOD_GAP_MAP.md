...
**Reconciled:** July 30, 2026
**Production baseline:** `f9050510`
**Evaluator evidence:** `a033133`
...
stopped before code because it could not meet its +3 causal gate. A third
typed-binding prerequisite then selected on only `4/6`, below its `5/6` gate.
The gold arm did not run, and the date family is closed on current evidence.
...
| QLoRA/rationale distillation | Verified-data Phases A and B accepted; training unstarted | Phase A proved the 512-row pipeline. Phase B then produced 4,096/4,096 schema-, execution-, flow-, and exact-state-verified rows across disjoint 3,072/512/512 splits, sixteen families, 32 structural signatures per family/split, and nine tool-order variants; independent rebuilds matched and Phase A stayed frozen | High for a chosen architecture | Only remaining route likely to move initial tool commitment materially | One predeclared adapter pilot and a new artifact | Strong only with fresh verified outcomes, act/abstain safety, argument accuracy, and closed-book/casual guardrails | Synthetic-grammar overfit, over-eager action, forgetting, quantization drift | Data infrastructure is eligible; require explicit campaign acknowledgement and freeze model, training budget, fresh development tasks, untouched validation, controls, gates, and rollback before QLoRA/SFT |
...
| User-approved plan routing | Tested twice and closed | A broad plan menu produced two verified wins but failed its gain/resource gates. A fresh typed source/outcome follow-up tied unchanged at `4/10`, produced `0/6` positive outcomes, and increased calls `13 -> 21` plus input tokens `11,019 -> 31,808` | High in principle | Use explicit user approval as a stronger orchestration signal | High for both tested menu mechanisms | Strong final-state and no-action controls | Labels and narrower menus did not carry evidence through multi-step execution | Close label/menu routing; require a materially different externally verified execution contract |
| Structured explicit-attachment evidence packet | Tested and rejected at development | A bounded evaluator-declared packet improved `4/10 -> 7/10` with three extraction wins, zero losses, `4/4` controls, and `10/10` validity, but calls rose `14 -> 17` and all three attachment-to-Wiki tasks had zero model-visible tools, zero tool calls, and unchanged external state | High for explicit attachment extraction; unproven for orchestration | Put small trusted-by-provenance, untrusted-by-content evidence directly in model context | Low prompt cost, but completion validation increased calls on correct extractions | Strong for extraction and controls; negative for verified state change | Easier extraction can still terminate before the pending approved outcome | Preserve as diagnostic evidence only; test a materially different verified completed-source receipt plus pending-outcome continuation on fresh tasks |
| Verified completed-source receipt and pending-outcome continuation | Tested and rejected at development | A fresh runtime-style receipt tied unchanged at `3/10`, produced zero paired wins and `0/6` verified state changes, passed `3/3` no-action controls, missed casual parity in both harness arms, and exposed zero tools on every positive. Calls rose only `13 -> 14` and all resource ceilings passed | High if exercised inside a real active tool loop; absent for prompt injection | Carry externally verified source state into the still-authorized outcome step | One bounded continuation call plus scoped tool context | Strong final-state and no-action scoring; prompt-level oracle was causally negative | A receipt rendered as prompt context cannot create an executable tool surface | Close further prompt receipts, labels, and menus; only reopen as a real in-loop continuation after successful source execution, with fresh tasks and a separate architecture contract |
| Active tool-loop post-source continuation | Tested and rejected at prerequisite | On six fresh positives, unchanged made a successful source call on only `3/6` versus the required `4/6`; two already reached verified mutation state, leaving only `1/6` source-success/outcome-stall case versus the required `3/6`. Controls were `3/3`, casual parity `1/1`, and validity `10/10` | Narrow: only turns already inside a successful source trajectory | Prevent premature finalization before an approved outcome | At most one continuation call | Strong causal prerequisite plus exact final state | Optimizes a rare late state while dominant failures occur before tool entry | Close this architecture seam on current evidence; target initial tool commitment in the separately governed verified-data lane |
...
| Deterministic date arithmetic | Closed after three disjoint slices | V1 gold reached `8/8`, but tool-only reached `2/8`. V2 tool-only reached `3/8` with `6/8` selection. V3 reached `1/6` with `4/6` selection; three selected calls had wrong arguments and two recurrences omitted the tool | High for explicit fixed-date requests | Exact calendar arithmetic once the whole semantic contract is correct | One tool round plus any parser/selector overhead | Strong for executed result; weak for full intent | A bundled vertical feature can overfit authored fixtures and add broad complexity | Close; reopen only from ordinary labeled outcomes, not more date-specific fixtures |
...
format-specific candidate. A fresh sparse-XLSX follow-up then scored `0/10`
...
Date specialization is closed on current evidence. V2 showed inconsistent
selection and three argument-binding misses; the fresh v3 prerequisite then
selected on only `4/6`, below its frozen binding-dominance gate. Gold and
...
postcondition, and date-specific candidates paused until ordinary labeled
outcomes expose a repeated open seam. The 57-outcome census remains below the
300-outcome complementarity gate for specialist escalation.

Approved-plan menu routing is also closed. The broad menu demonstrated two
verified wins without a safety loss but missed its gain and resource gates. Its
fresh typed source/outcome follow-up then tied unchanged on every outcome and
again exceeded call and token ceilings. Do not add the typed fields for routing
or tune smaller menus against either consumed cohort. A future plan-based
candidate must change the execution contract and verify step progression; more
labels are not a materially different mechanism.
