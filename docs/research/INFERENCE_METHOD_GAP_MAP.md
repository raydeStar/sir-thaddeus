...
| QLoRA/rationale distillation | Verified-data Phase A accepted; training unstarted | A deterministic fixed-catalog pipeline produced 512/512 shape-, execution-, and exact-state-verified trajectories with byte-identical rebuilds and disjoint 384/64/64 splits. The current four positive templates per family are pipeline proof, not training-ready diversity | High for a chosen architecture | Only remaining route likely to move initial tool commitment materially | Data expansion, training, and a new artifact | Strong only with fresh verified outcomes, act/abstain safety, and closed-book guardrails | Leakage, template overfit, over-eager action, forgetting, quantization drift | Expand under a new 3k-5k verified-data contract before any adapter pilot; do not claim a gain from Phase A |
