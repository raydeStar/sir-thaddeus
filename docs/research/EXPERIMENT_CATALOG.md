...
| --- | --- | --- | --- |
| Typed selected-target Wiki operations and verified read receipt | Harness capability and product quality | **Validated; pending protected delivery** | Raw LFM 1.2B and unchanged Thaddeus each scored `4/18` on a frozen disjoint bank; the candidate scored `17/18` with `8/8` reads, `5/6` writes spanning all operation families, `4/4` controls, thirteen paired wins, zero losses, exact activation, fewer calls/tokens, and lower p50/p95/VRAM. Development and exact repeat were `12/12`. Earlier read-operation v1 proved correct tool binding but failed synthesis; receipt v2 fixed reads but lacked inactive telemetry; v3 supplied the complete observable contract. This is model-agnostic harness uplift, not MMLU or frontier replacement. |
