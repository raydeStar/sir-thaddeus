...
This is a compact index of material Sir Thaddeus experiments through August 10,
...
| --- | --- | --- | --- |
| Verified file completion attestation v1 | Product quality | **Promoted through product PR `#330`** | With verified exact file effects already present, both arms retained canonical state at `8/8`; the runtime-owned attestation reduced provider calls `30 -> 18`, tokens by roughly 32 percent, and measured work time by roughly 33 percent. It is a completion-cost gain after external verification, not a new task-capability claim. |
| Exact verified file effects v1 | Harness capability | **Promoted through product PR `#329`** | Development, exact repeat, and disjoint validation each produced four paired wins and zero losses; fresh validation improved `4/8 -> 8/8`. Success depends on independently observed exact file state, not model confidence. |
| Adaptive deadline truthful receipt v1 | Product quality | **Promoted through product PR `#328`** | The bounded fallback preserved established state and added one fresh response-validity win while remaining truthful about what had and had not been verified. |
| Malformed provider protocol normalization v1 | Harness capability and product quality | **Promoted through product PR `#327`** | A model-identity-blind response-boundary normalizer raised the frozen five-case development slice from `0/5` to `3/5` with three wins and zero losses, reproduced the exact `3/5`, and reached the predeclared `3/5` on a disjoint sealed holdout with `5/5` validity and zero leaked protocol markers. It strips malformed channel wrappers and emits a Wiki receipt only for one exactly matched, independently successful versioned page update; it adds no model or tool calls. The two holdout misses required a verification reread or correct multi-source computation and were not converted into false success. |
| Exact-configuration forced-tool transport certification v1 | Harness capability and product quality | **Promoted through product PR `#319`** | Across 64 HTTP-valid raw episodes, single-visible-tool `auto` produced `32/32` exact native calls versus `8/32` for `required`, with 24 paired wins and zero losses. The real client-boundary certifier retained `required` for LFM 2.6B and 8B because existing strict/native parsing was effective, while selecting `auto` for Qwen 35B where required emitted rejected XML-like markup. All three activation calls were exact; probes execute no tool, ordinary chat makes no certification call, stale states fail closed, and policy contains no model identity or size branch. Full local tests passed 2,987 with two opt-in live skips; browser and casual-chat smokes passed. |
| Truthful blank Wiki-root outcome receipts v2 | Harness capability and product quality | **Promoted through product PR `#315`** | Blank drafts now become short truthful receipts only when a successful typed root result, a failed attempted root call, or explicit no-action intent establishes the outcome. Development and exact repeat improved `3/10 -> 8/10` with five wins and zero losses. Disjoint validation improved `2/14 -> 9/14`; five wins were causal receipt activations, two were inactive model variance, and there were no paired losses, false successes, state changes, or permission changes. The switch was removed, focused tests passed `21/21`, and the full local gate passed 2,960 tests with one skip. |
| Wiki-root referential selection v2 | Harness capability | **Validated, retained unmerged research; not promoted** | A clause-boundary selector repeated intended-write improvement `4/8 -> 7/8` with three wins and zero losses, then improved disjoint validation `6/10 -> 10/10` with four wins, zero losses, full validity, and exact `10/10` positive / `0/6` control activation. It failed its frozen resource gate because validation model calls rose `40 -> 42` and tool calls `20 -> 23`. Product SHA `6ce6d772` remains a labeled research candidate and the consumed holdout may not be retuned. |
| Terminal requested-value discipline v1 | Harness capability diagnostic | **Rejected at prerequisite; no product code** | The fresh 12-row screen found only three positives with successful requested evidence, and all three already ended on the requested value and passed strict scoring. With `0/3` qualifying terminal-distractor misses versus a frozen three-miss gate, the historical sealed-bank ordering failures did not transfer and do not authorize a global prompt or response rewriter. |
| Typed selected-target Wiki operations and verified read receipt | Harness capability and product quality | **Promoted through product PR `#307`** | Raw LFM 1.2B and unchanged Thaddeus each scored `4/18` on a frozen disjoint bank; the candidate scored `17/18` with `8/8` reads, `5/6` writes spanning all operation families, `4/4` controls, thirteen paired wins, zero losses, exact activation, fewer calls/tokens, and lower p50/p95/VRAM. Development and exact repeat were `12/12`. Earlier read-operation v1 proved correct tool binding but failed synthesis; receipt v2 fixed reads but lacked inactive telemetry; v3 supplied the complete observable contract. This is model-agnostic harness uplift, not MMLU or frontier replacement. |
...
| Approved-plan capability-menu oracle v1 | Evaluation infrastructure | **Rejected at development; broad plan-menu union closed** | The user-approved plan supplied directional signal: a broad local capability menu moved `3/10 -> 5/10` with two independently verified Wiki-state wins, zero losses, and all three no-action controls intact. It missed the frozen three-win floor, both harness arms missed casual parity, model calls rose `12 -> 38`, and input tokens rose `10,367 -> 179,743`. A richer typed plan contract remains a future design question; this menu is not a product candidate. |
| Typed approved-plan oracle v1 | Evaluation infrastructure | **Rejected at development; typed plan-menu routing closed** | On ten fresh tasks, raw, unchanged, typed-metadata-only, and typed-family-menu arms all scored `4/10`, with `0/6` positive outcomes, all four controls, and full validity. The candidate produced zero paired wins while increasing calls `13 -> 21` and input tokens `11,019 -> 31,808`, failing both hard resource ceilings. No repeat, validation, or product contract ran; plan-based follow-up now requires a materially different verified execution mechanism. |
| Structured attachment evidence packet v1 | Evaluation infrastructure | **Rejected at development; extraction signal only** | On ten fresh tasks, a bounded explicit-attachment packet improved unchanged `4/10 -> 7/10` with three extraction wins, zero losses, all four controls, and full validity. All three attachment-to-Wiki tasks exposed zero tools, made zero tool calls, and left state unchanged; calls also rose `14 -> 17`, failing the frozen non-increase gate. No repeat, validation, or product change ran. Evaluator PR `#142` preserves the evidence. |
| Verified source receipt continuation v1 | Evaluation infrastructure | **Rejected at development; prompt-level orchestration contract closed** | A bounded runtime-style receipt tied unchanged at `3/10`, with zero paired wins, `0/6` verified state changes, all three no-action boundaries preserved, and full validity. Every positive exposed zero tools and made zero tool calls; both harness arms also missed casual exact-response parity. Resource ceilings passed (`13 -> 14` calls), but no repeat, validation, or product change ran. Evaluator PR `#143` preserves the evidence. |
| Approved-plan active tool-loop continuation v1 | Evaluation infrastructure | **Rejected at prerequisite; late continuation seam closed** | Unchanged LFM 8B made a successful source call on `3/6` positives versus the frozen `4/6` minimum. Two already reached verified state, leaving only `1/6` source-success/outcome-stall case versus the required `3/6`; controls were `3/3`, casual parity `1/1`, and validity `10/10`. No product candidate or remaining arms ran. Gemma 12B Q4_K_XL was separately tool-ineligible under its current LM Studio Jinja template. Evaluator PR `#144`. |
| Grounded semantic-delta oracle v1 | Evaluation infrastructure | **Rejected at prerequisite; gold not run** | On ten fresh tasks, unchanged LFM 2.5 1.2B reached exact verified state on all `6/6` authorized typed updates with six successful writes, leaving zero positive headroom. Only `2/4` no-action or clarification controls preserved state and three forbidden write selections exposed a separate missing-value and ambiguous-target safety seam. Raw, direct, gold, repeat, validation, and product code did not run. |
| Verified tool trajectory data Phase A v1 | Model-adaptation infrastructure | **Infrastructure accepted; no training claim** | The current fixed-catalog generator produced 512/512 shape-, execution-, and exact-state-verified records across disjoint 384/64/64 splits, with balanced act/abstain families and byte-identical independent rebuilds. Answer-bearing rows remain ignored and tracked metadata is answer-blind. Four positive templates per family are not training-ready diversity; Phase B requires a separately frozen 3k-5k expansion before QLoRA/SFT. Evaluator PR `#145`. |
| Verified tool trajectory data Phase B v1 | Model-adaptation infrastructure | **Infrastructure accepted; adapter campaign separately gated** | The deterministic expansion produced 4,096/4,096 schema-, execution-, prerequisite-flow-, and exact-state-verified rows across disjoint 3,072/512/512 splits and sixteen balanced success, stop, and abstention families. Every family has 32 structural signatures and nine tool-order variants per split; independent rebuilds matched, Phase A remained frozen, and the full evaluator passed 585 tests with one skip. Answer-bearing data stays ignored. No model, judge, benchmark, or training calls ran. Evaluator PR `#146`. |
...
| MCPMark escalation qualification v1 | **Rejected at predeclared triage; diagnostic** | On six untouched filesystem-standard tasks, the Gemma 4 26B-A4B Q4_K_S escalation arm scored `0/3` valid and frozen Qwen passed none of its valid episodes; one successful Gemma mutation still failed verification on target/content. Escalation changed the failure layer, not the outcome. No harness machinery added; evaluator PRs `#181`, `#182`. |
| MCPMark WebArena shopping_admin feasibility v1 | **Rejected at predeclared stop rule; diagnostic** | Phase 0 proved the locally-hosted WebArena lane end to end with zero model calls (site image from the CMU mirror, socket-managed lifecycle, exact 22-tool catalog; evaluator PR `#183`, envelope-status fix `#184`). Stage-1 triage then scored `0/3` official on valid untouched easy tasks: every episode reached the live site, and failures were sustained browser-workflow depth, not source acquisition. Stage 2 not spent; standard/reddit/shopping remain untouched. Evaluator PR `#185`. |
| MCPMark WebArena escalation probe v1 | **Rejected; cross-model ceiling; diagnostic** | Gemma 4 26B-A4B Q4_K_S under unchanged Sir scored `0/3` official on the exact three tasks paired against Qwen's recorded `0/3`. Behavioral depth improved (login forms, successful typing) but no sustained-workflow completion. Two local families show the same boundary; escalation lane earns no follow-up from this probe. Evaluator PR `#187`. |
| MCPMark WebArena gold action shape oracle v1 | **Rejected; action shape insufficient; causal diagnostic** | On untouched `ny_expansion_analysis_easy`, an answer-blind skeleton (URL paths, menu labels, operation shape from zero-model site observation; no data values; `verify.py` never read) activated exactly (instruction + packet hashes matched) yet scored `0/1` where the unchanged baseline also failed `0/1`. The constraint is sustained action execution/perception, not action knowledge — reproducing the filesystem gold-skeleton lesson on a second family and closing MCPMark mechanism work absent a materially new hypothesis. Evaluator PR `#188`. |
| MCPMark WebArena state-observation oracle v1 | **Rejected at development; compact state insufficient** | On mechanically selected untouched standard task `sales_inventory_analysis`, unchanged Sir and the answer-blind state projection both scored `0/1` official with valid episodes and exact candidate activation. The oracle stopped after its first navigation (`1` turn, `1` tool call, 17,883 tokens) while unchanged authenticated and began catalog work (`7` turns, `8` calls, 125,088 tokens), so lower resource use is early failure rather than speed uplift. The proxy was removed; shared pinned-Chromium infrastructure remains. Evaluator PR `#192`. |
| MCPMark frontier cross-scaffold ceiling control v1 | **Blocked before provider calls; infrastructure ready** | Product PR `#325` and evaluator PR `#193` added generalized authenticated OpenAI-compatible transport with fail-closed missing-key handling and no secret values in command arguments. A different untouched WebArena standard task is mechanically selected and hash-locked for upstream MCPMark versus unchanged Sir under the same `gpt-5.6-sol` endpoint. No credential is configured, so zero sentinel or benchmark calls ran and no result is claimed. Any future stronger-model result is a ceiling/scaffold diagnostic, not fixed-model uplift. |
...
- Approved-plan capability-menu oracle:
  `experiments/manifests/approved-plan-tool-menu-oracle-v1.yaml` and
  `experiments/verdicts/2026-07-30-approved-plan-tool-menu-oracle-v1.md`.
...
- Verified tool trajectory data Phase A:
  `experiments/manifests/verified-tool-trajectory-data-phase-a-v1.yaml` and
  `experiments/verdicts/2026-07-30-verified-tool-trajectory-data-phase-a-v1.md`.
- Verified tool trajectory data Phase B:
  `experiments/manifests/verified-tool-trajectory-data-phase-b-v1.yaml` and
  `experiments/verdicts/2026-07-30-verified-tool-trajectory-data-phase-b-v1.md`.
