...
| Fail-closed unique local-file suffix resolution | Harness capability | **Promoted** | Resolve an incomplete read path only when one authorized-root file has the requested safe suffix. Fresh validation repeated at `12/16` versus `5/16`, with seven wins, zero losses, full validity, zero negative activations, fewer calls/tokens, and lower latency. Explicit `./name`, ambiguity, unsafe syntax, and writes do not receive recursive assistance. |
