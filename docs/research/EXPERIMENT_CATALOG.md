...
| --- | --- | --- | --- |
| Wiki-root temporal-deferral pruning | Harness capability and product safety | **Protected product PR `#248`** | A root-scoped deterministic policy reproduced `4/16 -> 11/16` in development and exact repeat with seven wins and zero losses. Disjoint validation improved `4/18 -> 13/18` with nine wins, zero losses, `10/10` deferred safety, `8/8` immediate root-tool reachability, full validity, and fewer calls. This improves mutation precision, not MMLU. |
