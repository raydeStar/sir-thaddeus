...
This is a compact index of material Sir Thaddeus experiments through August 8,
...
| --- | --- | --- | --- |
| Malformed provider protocol normalization v1 | Harness capability and product quality | **Validated locally; protected delivery pending** | A model-identity-blind response-boundary normalizer raised the frozen five-case development slice from `0/5` to `3/5` with three wins and zero losses, reproduced the exact `3/5`, and reached the predeclared `3/5` on a disjoint sealed holdout with `5/5` validity and zero leaked protocol markers. It strips malformed channel wrappers and emits a Wiki receipt only for one exactly matched, independently successful versioned page update; it adds no model or tool calls. The two holdout misses required a verification reread or correct multi-source computation and were not converted into false success. |
| Exact-configuration forced-tool transport certification v1 | Harness capability and product quality | **Validated locally; protected delivery pending** | Across 64 HTTP-valid raw episodes, single-visible-tool `auto` produced `32/32` exact native calls versus `8/32` for `required`, with 24 paired wins and zero losses. The real client-boundary certifier retained `required` for LFM 2.6B and 8B because existing strict/native parsing was effective, while selecting `auto` for Qwen 35B where required emitted rejected XML-like markup. All three activation calls were exact; probes execute no tool, ordinary chat makes no certification call, stale states fail closed, and policy contains no model identity or size branch. Full local tests passed 2,987 with two opt-in live skips; browser and casual-chat smokes passed. |
| Truthful blank Wiki-root outcome receipts v2 | Harness capability and product quality | **Validated locally on product SHA `4ba03c0b`; protected delivery pending** | Blank drafts now become short truthful receipts only when a successful typed root result, a failed attempted root call, or explicit no-action intent establishes the outcome. Development and exact repeat improved `3/10 -> 8/10` with five wins and zero losses. Disjoint validation improved `2/14 -> 9/14`; five wins were causal receipt activations, two were inactive model variance, and there were no paired losses, false successes, state changes, or permission changes. The switch was removed, focused tests passed `21/21`, and the full local gate passed 2,960 tests with one skip. |
...
| Typed selected-target Wiki operations and verified read receipt | Harness capability and product quality | **Validated; protected delivery PR `#307`** | Raw LFM 1.2B and unchanged Thaddeus each scored `4/18` on a frozen disjoint bank; the candidate scored `17/18` with `8/8` reads, `5/6` writes spanning all operation families, `4/4` controls, thirteen paired wins, zero losses, exact activation, fewer calls/tokens, and lower p50/p95/VRAM. Development and exact repeat were `12/12`. Earlier read-operation v1 proved correct tool binding but failed synthesis; receipt v2 fixed reads but lacked inactive telemetry; v3 supplied the complete observable contract. This is model-agnostic harness uplift, not MMLU or frontier replacement. |
