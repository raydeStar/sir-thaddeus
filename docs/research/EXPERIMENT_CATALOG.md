...
| --- | --- | --- | --- |
| Wiki-root non-action tool pruning | Harness capability and product safety | **Validated in protected product PR `#246`** | Reuse the shipped deterministic non-action policy to withhold only `wiki_root_create` while preserving read-only tools and upstream forced-tool decisions. Fresh v2 development and exact repeat each improved `7/12 -> 9/12` with two wins and zero losses; disjoint validation improved `10/16 -> 13/16` with three wins, zero losses, `8/8` non-action correctness, full validity, correct activation, and fewer model calls. This improves tool precision, not MMLU. |
